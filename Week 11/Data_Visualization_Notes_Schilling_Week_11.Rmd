...
General rule: avoid creating dual-axis charts, particularly with time series data

```{r two-y-axes}

# dataset of monetary base and S&P 500 over time with indexed scores
head(fredts) 
...
# Put the plots together use cowplot
cowplot::plot_grid(p1, 
                   p2, 
                   nrow = 2, 
                   rel_heights = c(0.75, 0.25), # show p1 on 75%, p2 on 25%
                   align = "v")
...
# The dataset - revenue and employees at Yahoo by year; binary Mayer as CEO
head(yahoo)

# Trajectory of employees vs revenue over time
p <- ggplot(data = yahoo,
            mapping = aes(x = Employees,
                          y = Revenue))
...
# Compute the ratio of revenue to employees and make a line graph
p <- ggplot(data = yahoo,
            mapping = aes(x = Year,
                          y = Revenue / Employees)) 

p + geom_vline(xintercept = 2012) + # mark when Mayer became CEO
  ...
  labs(x = "Year\n",
       y = "Revenue/Employees",
       title = "Yahoo Revenue to Employees Ratio, 2004-2014") +
  theme_bw()
...
# Dataset - student debt comparisons
head(studebt)

# Redraw the pie charts to compare the distributions

# Labels
p_xlab <- "Amount Owed, in thousands of Dollars"
p_title <- "Oustanding Student Loans"
p_subtitle <- "44 million borrowers ow a total of $1.3 trillion"
...
# Create faceted horizontal bar plot
p <- ggplot(data = studebt,
            mapping = aes(x = Debt,
                          y = pct/100, # make percents decimals
                          fill = type))

p + geom_bar(stat = "identity") + # could have also used geom_col()
  scale_fill_brewer(type = "qual", palette = "Dark2") +
  scale_y_continuous(labels = scales::percent) +
  guides(fill = FALSE) + # don't include the legend
  labs(y = "",
       x = p_xlab,
       caption = p_caption,
       title = p_title,
  ...
  theme_bw() +
  theme(
    # bold the facet labels and left-align them
    strip.text.x = element_text(face = "bold", hjust = 0),
    # remove the background from the facet labels
    strip.background.x = element_blank()) 

# Redraw the pie charts as stacked bar graphs
...
# By changing the mapping, we can make a stacked bar plot
p <- ggplot(data = studebt,
            mapping = aes(y = pct/100,
                          x = type,
                          fill = Debtrc)) 

p + geom_bar(stat = "identity", 
             color = "grey80") + # outline the bars in light gray
  scale_x_discrete(labels = as_labeller(f_labs)) +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_viridis(discrete = TRUE) + # color palette
  # Custom formatting of the legend; align lightest colors to smallest values
  ...
  theme_bw() +
  theme(legend.position = "top",
        # Format y-axis text (bold, right-align, size 12)
...
This section is not in the book and is taken from [Time Based Heatmaps in R](https://www.littlemissdata.com/blog/heatmaps). `geom_tile()` can be used to create heat maps.
